...
Original file is located at
    https://colab.research.google.com/drive/11s2oMmv37kwwHAthU1tBxECLQIUHudZ6
"""
...
data_path = 'cmn2.txt'# 'cmn-eng/cmn.txt'
...
def mysplit(s): #負責切割英文字串(但保留標點符號)
...
for line in lines[: min(num_samples, len(lines) - 1)]:
    ...
    input_texts.append(input_text)
    target_texts.append(target_text)
    for word in mysplit(input_text):
...
max_encoder_seq_length = max([len(mysplit(txt)) for txt in input_texts])
max_decoder_seq_length = max([len(txt) for txt in target_texts])
embedding_code_length = 64
...
for i, (input_text, target_text) in enumerate(zip(input_texts, target_texts)):
    for t, word in enumerate(mysplit(input_text)):
...
encoder_embedding_inputs = Embedding(num_encoder_tokens, embedding_code_length, input_length=max_encoder_seq_length)(encoder_inputs)
encoder = LSTM(latent_dim, return_state=True)
